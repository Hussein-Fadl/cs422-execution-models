...
class LazyFetchFilterRule protected (config: RelRule.Config)
  ...
  override def onMatchHelper(call: RelOptRuleCall): RelNode = {
    val stitch1: RelNode = call.rel(1)
    val filter: LogicalFilter = call.rel(2)
    val scan: LateColumnScan = call.rel(3)

    val fetch: LogicalFetch = new LogicalFetch(stitch1, scan.getRowType, scan.getColumn , None)

    val newCondition = RexUtil.shift(
      filter.getCondition,
      stitch1.getRowType.getFieldCount
    )

    filter.copy(filter.getTraitSet, fetch, newCondition)
