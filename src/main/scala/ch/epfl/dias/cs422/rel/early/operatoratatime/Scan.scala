...
class Scan protected(
  ...
  def execute(): IndexedSeq[Column] = {
    val count = getRowType.getFieldCount

    val cols = for(i <- 0 until count) yield  scannable.getColumn(i).toIterable.toIndexedSeq

    cols :+ (0 until cols.head.size).map(_ => true)
